...
log_file=stencil_16to16_results.csv
rm -f $log_file
...
# Target: Chebyshev filtering stencil (16-packed in → 16-packed out)
NI=56
NJ=56
NK=56
NB=616
...
for ti in 8 16 28 56; do
    for tj in 8 16 28 56; do
        for tb in 16 32 64 128 256 9999; do
            ...
            sme++ -O3 -fopenmp -I./include -DMODE=16 -DITERS=3 \
                -DSTENCIL_TI=$ti -DSTENCIL_TJ=$tj -DSTENCIL_TB=$tb \
                stencil_extended_bench.cpp -o stencil_16to16_bench 2>&1

            if [ $? -eq 0 ]; then
                OMP_NUM_THREADS=38 mpirun -np 1 --map-by ppr:1:numa:PE=38 \
                    numactl --cpunodebind=0 --membind=1 \
                    ./stencil_16to16_bench $NI $NJ $NK $NB >> $log_file 2>&1
            else
                echo "$ti,$tj,$tb,$NI,$NJ,$NK,$NB,16,BUILD_FAILED,0,0" >> $log_file
            fi
        done
    done
done
...
echo ""
echo "Stencil 16→16 sweep complete. Results in $log_file"
