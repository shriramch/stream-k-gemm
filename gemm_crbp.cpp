// SME GEMM Implementation with Cache Blocking and Packed A/B
// Packed format:
//   A_packed: [tiles_m][K][ZA_TILE_M] - K panels of ZA_TILE_M per M tile
//   B_packed: [tiles_n][K][ZA_TILE_N] - K panels of ZA_TILE_N per N tile
//   C/D: row-major [M][N] with stride N (NOT packed)
//
// D = alpha * A * B + gamma
...
class gemm {
 public:
  ...
  // =========================================================================
  // Micro-kernel for PACKED A/B: contiguous access, no strides
  // =========================================================================
  static __forceinline void microkernel_packed(
  ...
  // =========================================================================
  // Load partial C from ROW-MAJOR (stride N)
  // =========================================================================
  ...
  // =========================================================================
  // Store partial C to ROW-MAJOR (no scaling)
  // =========================================================================
  ...
  // =========================================================================
  // Store final C to ROW-MAJOR with scaling: D = alpha * ZA + gamma
  // =========================================================================
    ...
    for (int kc = 0; kc < K; kc += KC) {
      ...
      for (int nc = 0; nc < tiles_n; nc += nc_tiles) {
        ...
        for (int tn_local = 0; tn_local < tn_end; ++tn_local) {
          ...
          microkernel_packed(k_len, a_ptr, b_ptr, ptrue);
  ...
  // =========================================================================
  // Main GEMM: packed A/B, row-major C (NO SME attributes here)
  // A_packed: [tiles_m][K][ZA_TILE_M]
...
#pragma omp parallel for
  ...
  // =========================================================================
  // Pack A from column-major to [tiles_m][K][ZA_TILE_M]
  // =========================================================================
  ...
  // =========================================================================
  // Pack B from row-major to [tiles_n][K][ZA_TILE_N]
  // =========================================================================
  ...
  // Test wrapper with packing (NO SME attributes)
...
bool test_gemm(int M, int N, int K, T alpha, T gamma) {
  ...
  printf("Testing GEMM<%s> PACKED: M=%d N=%d K=%d alpha=%.2f gamma=%.2f\n",
